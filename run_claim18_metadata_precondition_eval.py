#!/usr/bin/env python3
"""Run CLAIM-18 independent metadata-precondition check."""
...
SCENARIOS_PATH = ROOT / "external_scenarios" / "claim18_metadata_precondition_independent_v0_1.json"
DEFAULT_RESULTS_JSON = ROOT / "results" / "claim18_metadata_precondition_independent_v0_1_results.json"
DEFAULT_RESULTS_MD = ROOT / "results" / "claim18_metadata_precondition_independent_v0_1_results.md"
...
def evaluate() -> dict[str, Any]:
    ...
    return {
        "status": "CLAIM-18 metadata precondition independent check. Internal, not benchmark-grade.",
...
def render_markdown(output: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Scenario Rows",
            "",
            "| Strategy | Scenario | Expected | Selected role | Trap | Action | Act ok | Trap fail | FC | OB |",
            "|---|---|---|---|---|---|---|---|---|---|",
        ]
    )
    for row in output["rows"]:
        lines.append(
            ...
            f"{row['selected_role']} | {row['selected_trap']} | {row['action']} | "
